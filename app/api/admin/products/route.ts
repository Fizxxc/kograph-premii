...
export async function POST(request: Request) {
  try {
    ...
    if (!name || !category || !description || !image || price < 0 || sold_count < 0 || stockInput < 0) {
    ...
    const { error: uploadError } = await admin.storage.from("product-images").upload(fileName, fileBuffer, {
