...
export async function GET() {
  ...
  let query = auth.admin.from("live_chat_rooms").select("id, title, status, product_id, customer_user_id, assigned_admin_id, last_message_at, created_at, products(name)").order("last_message_at", { ascending: false });
...
export async function POST(request: Request) {
  ...
  try {
    ...
    const { data: product } = await auth.admin.from("products").select("id, name, live_chat_enabled, support_admin_ids").eq("id", productId).single();
    if (!product || !product.live_chat_enabled) return NextResponse.json({ error: "Live chat belum aktif untuk produk ini" }, { status: 400 });

    const { data: room, error: roomError } = await auth.admin.from("live_chat_rooms").insert({
      product_id: product.id,
      customer_user_id: auth.user.id,
      telegram_chat_id: (auth as any).profile?.telegram_id || null,
