...
export default async function HomePage({
  ...
  if (inStock) query = query.or("service_type.eq.pterodactyl,stock.gt.0");
  ...
  else if (sort === "sold_desc") query = query.order("sold_count", { ascending: false });
    ...
    <div className="space-y-10">
      ...
      <section className="grid gap-4 md:grid-cols-3">
        <div className="rounded-3xl border border-white/10 bg-white/5 p-5">
          <div className="text-sm text-slate-400">Produk aktif</div>
          <div className="mt-2 text-3xl font-bold text-white">{products?.length ?? 0}</div>
        </div>
        <div className="rounded-3xl border border-white/10 bg-white/5 p-5">
          <div className="text-sm text-slate-400">Layanan panel</div>
          <div className="mt-2 text-3xl font-bold text-white">
            {products?.filter((item: any) => item.service_type === "pterodactyl").length ?? 0}
          </div>
        </div>
        <div className="rounded-3xl border border-white/10 bg-white/5 p-5">
          <div className="text-sm text-slate-400">Akun premium & saldo</div>
          <div className="mt-2 text-sm leading-7 text-slate-300">
            Lebih aman, lebih rapi, dan ada jalur report jika terjadi masalah saldo.
          </div>
        </div>
      </section>
      ...
      <section id="catalog" className="space-y-6">
        <div className="flex items-end justify-between gap-3">
          <div>
            ...
            <h2 className="mt-2 text-3xl font-bold text-white">Produk Premium & Panel</h2>
          </div>
